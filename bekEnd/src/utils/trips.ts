...
export function getAllTrips(): Partial<Trip>[] {
  return trips.map(trip => ({id: trip.id, name: trip.name,destination: trip.destination,startDate: trip.startDate,endDate:trip.endDate,image:trip.image , activities:trip.activities}));
}
